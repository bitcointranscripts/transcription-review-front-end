...
interface SuggestSourceParams extends Pick<TranscriptMetadata, 'title' | 'media'> {
    targetRepository: string
}
...
const saveProgress = async ({
    ...
    const { srcRepo, srcDirPath, filePath, fileNameWithoutExtension } = resolveGHApiUrl(transcriptUrl);
...
const submitReview = async ({
    ...
    await githubApi.post('/pr', {
        ...
        It is not part of the evaluation process, it updates automatically and it will be merged after the [review submission PR](${prResult.data.html_url}) is merged.`,
        head: `${srcOwner}:${srcBranch}`,
        // hacky way to avoid for now to keep extra information about the metadata repo in the db
...
const suggestSource = async ({
    ...
    await githubApi.post('/newBranch', {
        ...
        baseBranch: process.env.NEXT_PUBLIC_VERCEL_ENV === "production" ? "master" : "staging",
        branchName
    })
    ...
    const prResult = await githubApi.post('/pr', {
        ...
        // TODO: needs better handling of base branch
        base: process.env.NEXT_PUBLIC_VERCEL_ENV === "production" ? "master" : "staging",
    });
    return prResult.data.html_url
}
