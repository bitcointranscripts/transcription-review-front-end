...
const TranscriptEditor = ({ reviewData }: { reviewData: TranscriptReview }) => {
  ...
  const [date, setDate] = useState(new Date(reviewData.transcript.metadata.date))
  ...
  const { isOpen: restoreModalIsOpen, onOpen: openRestoreModal, onClose: closeRestoreModal } = useDisclosure();
  const { isOpen: submitModalIsOpen, onOpen: openSubmitModal, onClose: closeSubmitModal } = useDisclosure();
  const {
  ...
  const saveTranscript = async (edits: any, speakers: string[]) => {
  ...
  return (
    <>
      ...
      />
      <RestoreOriginalModal isOpen={restoreModalIsOpen} onClose={closeRestoreModal} onClick={restoreOriginalContent} />
